...
### Well-Known Events (`src/well-known-events.ts`)

Defines the `wellKnownSchemas` registry: a `const` object mapping all 24 well-known event name strings to their protobuf `*PropertiesSchema` descriptors imported from `@buf/fivebits_cotton.bufbuild_es`. This registry is the single source of truth — `WellKnownEventName` (the union of all well-known event names) and `WellKnownEventPropsMap` (mapping each name to its `MessageInitShape`) are both derived from `typeof wellKnownSchemas`. Also defines `TrackFn` (an overloaded function type: the first overload narrows properties for well-known events, the second accepts any `string` event with loose `Record<string, JsonValue>` props) and `TrackOptions`. Re-exports `JsonValue` from `@bufbuild/protobuf`.

### Event Creation (`src/track.ts`)

`toEvent(projectId, kind, sessionId, distinctId, props?, opts?)` builds a protobuf `Event` object from event kind, properties, and options. The `customProperties` field is a typed `map<string, common.v1.PropertyValue>` — a oneof over `string`, `int64`, `double`, `bool`, and `Timestamp`. Two paths populate it:

- **Well-known events** (kinds present in `wellKnownSchemas`): `validateWellKnownProps` partitions input into known and extra keys, runs `create()` + `@bufbuild/protovalidate` on the known keys, then returns `{ msg, extras }` (no `toJson` round-trip). `buildKnownPropertyMap` walks `schema.fields` and uses `scalarToPropertyValue` to build a `PropertyValue` for each non-default scalar, picking the oneof case from the field's proto scalar type so that e.g. an integer-valued `double` field still serializes as `doubleValue`. Default-valued scalars (`''`, `0`, `false`, `0n`) are skipped to mirror implicit-presence wire semantics. Extras are then merged in via `jsValueToPropertyValue`. If validation fails, the event is dropped (returns `null`) with a contextual error log.
- **Custom events**: every entry in `props` flows through `jsValueToPropertyValue`. The mapping is: `string` → `stringValue` (truncated to 1024 code points with a warn log if longer — matches the `string.max_len = 1024` validator, which CEL's `size()` measures in code points); `boolean` → `boolValue`; finite `number` → `intValue` if `Number.isInteger && Number.isSafeInteger` else `doubleValue` (non-finite numbers dropped); `bigint` → `intValue`; `Date` → `timestampValue` via `timestampFromMs(d.getTime())`; arrays/objects → `JSON.stringify` then string policy; `null`/`undefined` and non-serializable circular structures dropped (the property is dropped, not the event).

Auto properties include: `$projectId`, `$url`, `$referrer`, `$locale`, `$screenWidth`, `$screenHeight`, `$pageTitle`, `$sdkVersion`, UA Client Hints when available (`$browser`, `$browserVersion`, `$os`, `$osVersion`, `$device`, `$mobile`), and any present UTM params. `sessionId` and `distinctId` are set as top-level fields on the `Event` proto (their own ClickHouse columns), not properties. `distinctId` is the profile identifier — `externalId` after `identify()`, otherwise the anonymous ID. After building the `Event` proto, `toEvent` also validates the full `Event` object against `EventSchema` via protovalidate; invalid events are dropped with an error log. Re-exports `TrackFn`, `TrackOptions`, `JsonValue`, `WellKnownEventName`, and `WellKnownEventPropsMap` from `well-known-events.ts`.
...
- `setupNotificationClickTracking(track)` — sets up `notification_clicked` tracking across two cases: (1) page already open: listens for a `cotton_notification_click` postMessage from the SW; (2) page opened by the click: reads `?cotton_nc=<JSON>` from the URL, calls `track`, strips the param with `history.replaceState`. Returns a cleanup function.
...
- Barrel export: `src/index.ts` re-exports `init`, `destroy`, `track`, `reset`, `identify`, `rotate` and types `CottonConfig`, `InitOptions`, `BatchConfig`, `TrackOptions`, `SessionConfig`, `JsonValue`, `JsonObject`, `TrackFn`, `WellKnownEventName`, `WellKnownEventPropsMap`; and from `push.ts`: `subscribePush`, `unsubscribePush`, `setupNotificationClickTracking`, `PushOptions`. `JsonValue` and `JsonObject` are re-exported from `@bufbuild/protobuf`. Deprecated alias `CottonEventName` (→ `WellKnownEventName`) is re-exported for backward compatibility. Internal module functions and implementation details are not publicly exported.
