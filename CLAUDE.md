...
bun run build          # Stamp src/version.ts (prebuild), then compile TypeScript to dist/ with tsc
...
- `make protos` — `buf generate` (local `protoc-gen-es`, `target=ts`, `import_extension=js`; config in `buf.gen.yaml`) from the committed `proto/` into `src/gen/` (offline, no BSR), then runs `make typed-events`. `src/gen` compiles with the rest of `src` under the strict tsconfig; google WKTs resolve from `@bufbuild/protobuf/wkt` and are not emitted, while `buf/validate` **is** emitted (the pug protos import its field-option extensions via a relative path).
- `make typed-events` — regenerates `src/well-known-events.generated.ts`. The script introspects a throwaway compile of `src/gen` (`tsc -p tsconfig.gen.json` → `.codegen-tmp`, gitignored) because node cannot import the `.ts` source directly (protobuf-es enums + `.js`-extension imports). Output is deterministic; CI regenerates and diffs it.
...
`createTransport(endpoint, apiKey)` returns an object with `send`, `sendBatch`, and `beacon` methods. It uses ConnectRPC with protobuf serialization via the vendored generated code in `src/gen` (see Protobuf Codegen). The `beacon` method uses `navigator.sendBeacon` with binary protobuf for reliable delivery during page unload; since `sendBeacon` cannot carry request headers, the API key is appended as a `?api_key=` query parameter on the beacon URL.
