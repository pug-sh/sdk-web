...
- `isIdentified()` / `markIdentified(id)` — tracks whether the user has been identified. Derived from persisted `externalId` on page load (so `true` if a previous `identify()` succeeded). `markIdentified` persists the `externalId` to localStorage so it survives page reloads. Controls whether `anonymousId` is sent on the next `identify()` RPC (first call triggers server-side merge of anonymous → identified profile; subsequent calls skip merge).
