...
Owns the in-memory consent state (`TrackingConsent`: `'granted' | 'denied'`). It is intentionally not persisted; callers that need consent across reloads should pass `defaultTrackingConsent` according to their own consent storage on each `init()`. `pug.ts` gates listener attachment on consent and stores the desired `autoCapture` selection separately so `setAutoCapture()` can be called before opt-in.
