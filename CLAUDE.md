...
- `clearProfile()` — clears storage (both anonymous ID and external ID) and resets identified state, logging an error per key whose removal cannot be confirmed (an unremoved shared cookie would otherwise resurface the identity). Called by `pug.reset()`.
