...
- Barrel export: `src/index.ts` re-exports `init`, `destroy`, `track`, `reset`, `identify`, `rotate` and types `CottonConfig`, `InitOptions`, `BatchConfig`, `TrackOptions`, `SessionConfig`, `JSONValue`, `TrackFn`, `WellKnownEventName`, `WellKnownEventPropsMap`; and from `push.ts`: `subscribePush`, `unsubscribePush`, `setupNotificationClickTracking`, `PushOptions`. Deprecated aliases `CottonEventName` (→ `WellKnownEventName`), `JsonValue`/`JsonObject` (→ `JSONValue`) are re-exported for backward compatibility. Internal module functions and implementation details are not publicly exported.
